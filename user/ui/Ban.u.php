...
class BanUi {
	 ...
	 *
	 */
	public function getForm(bool $active): string {
		...
		$h .= '<form><label><input type="checkbox" data-field="active" value="'.($active ? 'checked' : '').'"/> '.s("Actifs").'</label></form>';
	...
	public function create(User $eUser, $userToBanIp, $nUserOnIp): \Panel {
		...
		$type = $form->checkbox('type[]', 'user', ['display' => fn($input) => $input.' '.s("Utilisateur")]);
		...
		if($userToBanIp === NULL) {
			...
			$type .= $form->checkbox('type[]', 'ip', ['display' => fn($input) => $input.' '.s("Adresse IP").' '.p("({value} personne connectée)", "({value} personnes connectées)", $nUserOnIp)]);
