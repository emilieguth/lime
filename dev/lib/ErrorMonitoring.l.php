...
class ErrorMonitoringLib {
	 ...
	 * Get an error by ID
	 */
	public static function getById($id): \Element {
	 ...
	 *
	 */
	public static function getLast(int $offset, int $limit, ?string $message, ?string $mode, ?bool $withUnexpected, \user\User $eUser): array {

		if($message !== NULL) {
			Error::model()->whereMessage('LIKE', '%'.$message.'%');
		}

		if($withUnexpected === FALSE) {
			Error::model()->whereType('!=', Error::UNEXPECTED);
		}

		if($eUser->notEmpty()) {
			Error::model()->whereUser($eUser);
		}
