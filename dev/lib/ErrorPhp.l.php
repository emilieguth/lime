...
class ErrorPhpLib {
	...
	public static function clean() {
		...
		$date = \util\DateLib::subFromDate(Error::model()->now(), DevConf::KEEP_ERROR.' day');

		$eError = Error::model()
			->select(['id', 'createdAt'])
			->sort(['createdAt' => 'DESC'])
			->getCollection(DevConf::MAX_ERROR, 1)
			->current();

		$date2 = $eError['createdAt'] ?? NULL;

		if($date2 === NULL or \util\DateLib::compare($date, $date2) > 0) {
			$eError = Error::model()
				->select(['id'])
