...
class CronLib {
	...
	/**
	 * Save a cron execution in the database
	 */
	public static function saveBegin(): Cron {
		...
		$eCron = new Cron([
			'request' => LIME_REQUEST_PATH,
		]);

		Cron::model()->insert($eCron);
	...
	/**
	 * Save the end of a cron execution in the database
	 */
	public static function saveEnd(Cron $eCron) {
		...
		if($lastError) {
			...
			$status = Cron::ERROR;
			$error = '['.ErrorPhpLib::getCode($lastError['type']).'] '.$lastError['message'].' in '.$lastError['file'].' on line '.$lastError['line'];
