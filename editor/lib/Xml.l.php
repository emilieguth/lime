...
class XmlLib {
	...
	public function fromHtml(?string $value, array $options = []): ?string {
		...
		while ($i < $main->childNodes->length) {
			...
			switch($node->nodeName) {
				...
				case 'p' :
					$this->cleanParagraphNode($node);
					break;

				case 'ol' :
				case 'ul' :
					$this->cleanListNode($node);
					break;

				case ($this->options['acceptFigure'] ? 'figure' : NULL) :
					$i += $this->cleanFigureNode($node);
					break;
	...
	protected function cleanFigureNode(\DOMNode $node) : int {
		...
		// Clean grammarly
		$this->cleanGrammarlyChildren($node);
		...
		if($node->getAttribute('interactive') === 'true') {
			...
			if(
				$node->hasAttribute('style') and
				in_array($node->getAttribute('style'), ['default', 'space', 'stick', 'window', 'black-window', 'film', 'polaroid', 'porthole', 'dynamic', 'black-blocks']) === FALSE
			) {
	...
	protected function createQuote(\DOMNode $node) : \DOMNode {
		...
		while($node->childNodes->length > 0) {
			...
			if($childNode->nodeName !== 'p') {
				...
				$newChildNode = $this->cleanParagraphNode($childNode);
	...
	protected function getQuoteNode(\DOMNode $node) {
		...
		// Clean grammarly
		$this->cleanGrammarlyChildren($nodeDiv);
	...
	protected function cleanGrammarlyChildren(\DOMNode $node) {

		for($i = 0; $i < $node->childNodes->length; $i++) {

			$childNode = $node->childNodes[$i];

			if($this->cleanGrammarlyNode($childNode)) {
				$i--;
			}
	...
	protected function cleanGrammarlyNode(\DOMNode $node): bool {

		if($node->nodeName === 'grammarly-btn') {
			$node->parentNode->removeChild($node);
			return TRUE;
		} else {
			return FALSE;
	...
	protected function createMediaNode(\DOMNode $node, string $tagName, array $attributes = []): int {
		...
		foreach($attributes as $from => $to) {
			$newNode->setAttribute($to, $node->getAttribute(trim($from, '?')));
		}
	...
	protected function createParagraphElement(\DOMNode $node, array $attributes) : \DOMNode {
	 ...
	 * @param \DOMNode $node
	 */
	protected function cleanListNode(\DOMNode $node) {
