class GalleryEditor {
	...
	applyGallerySize(rows) {
		...
		if(
			(this.selector.getAttribute('data-size') === 'expanded') ||
			(
				this.options.canExceedMax() === true &&
				this.selector.getAttribute('data-size') !== 'left' &&
				this.selector.getAttribute('data-size') !== 'right'
			) ||
			(this.selector.getAttribute('data-size') === 'compressed' && window.matchMedia('(max-width: 767px)').matches)
		) {
