...
class Editor {
	...
	static selection() {
		...
		setInterval(() => {
			...
			for(let key in this.instances) {
				...
				const instanceId = this.instances[key];
				...
				if(
					...
					qsa(instanceId +' blockquote[contenteditable="true"]:focus').length > 0 ||
	...
	// returns top ancestor within the contenteditable of a node
	static getTopAncestor(node) {

		let nodeCheck = node;

		while(nodeCheck.parentElement) {

			if(this.isEditorNode(nodeCheck.parentElement)) {
				return nodeCheck;
			}
...
class EditorKeyboard {
		...
		if(previousNode === null) {
			...
			switch(previousNode.tagName) {
				...
				case 'FIGURE' :
					...
					const topNode = Editor.getTopAncestor(selection.getRangeAt(0).startContainer);
	...
	static list(nodeExisting, type) {
		...
		const nodeRoot = nodeExisting.parentElement;
		...
		nodeRoot.replaceChild(nodeBr, nodeExisting);
		...
		if(browser.isFirefox) {
			nodeBr.parentElement.replaceChild(document.createElement('br'), nodeBr);
		}
	...
	static isEmptyLine(nodeCheck) {
		...
		const nodeAncestor = Editor.getTopAncestor(nodeCheck);

		if(nodeAncestor === null || nodeAncestor.tagName !== 'P') {
			return false;
		}
	...
	static getTextNodeFromPreviousLine(nodeCurrent) {
		...
		const nodeCurrentAncestor = Editor.getTopAncestor(nodeCurrent);
	...
	static checkLine(nodeCheck, text) {
		...
		while(
			...
			// Stop looping
			node &&
			Editor.isEditorNode(node.parentElement) === false
		) {
...
// Handle <b>, <u>, <i>...
document.delegateEventListener('click', '.editor-action', function(e) {
...
document.delegateEventListener('submit', '.editor-action-form', function(e) {
...
document.delegateEventListener('submit', '#editor-media-configure', function(e) {
	...
	const url = qs('#editor-media-configure input[data-field="url"]').value;

	qs('div[data-url="' + url + '"]', media => {
		media.setAttribute('data-title', qs('#editor-media-configure input[data-field="title"]').value);
		media.setAttribute('data-license', qs('#editor-media-configure select[data-field="license"]').value);
		media.setAttribute('data-source', qs('#editor-media-configure input[data-field="source"]').value);
	});
...
class EditorFormat {
	...
	static start(instanceId) {
		...
		let html = '<div id="'+ instanceId.substring(1) +'-format">';
			...
			if(instance.getAttribute('data-text-full') === '1') {
			...
			html += '</div>'+
				'<div class="editor-box-selection-link">'+
					'<form class="editor-action-form" data-action="link" data-instance="'+ instanceId +'">'+
						...
						'<a class="editor-action" data-action="link-submit" data-instance="'+ instanceId +'"></a>'+
			...
			if(instance.getAttribute('data-figure') === '1') {
				...
				html += '<div id="'+ instanceId.substring(1) +'-box-line" class="editor-box-line" data-instance="'+ instanceId +'">'+
					'<div class="editor-box-line-open">'+
						...
						'<div class="editor-box-line-helper">'+ instance.getAttribute('data-placeholder-empty') +'</div>'+
					'</div>'+
	...
	static displayMedia(instanceId, figureId, mediaSelector) {
		...
		let html = '<div class="editor-box-media">';
			...
			if(
				...
				qsa(figureId +' > [data-source="twitter"]').length === 0 // Can not move twitter cards due to an iframe bug in jquery sortable
			) {
	...
	static updateLine(instanceId, range) {
		...
		if(range !== null) {
			...
			const selectorCheck = Editor.getTopAncestor(range.commonAncestorContainer);

			// Focus is on an other editor
			if(selectorCheck !== null && selectorCheck.parentElement.id === instance.id) {
		...
		if(
			selector === null ||
			selector.tagName === 'FIGURE' ||
			(
				EditorKeyboard.isEmptyLine(selector) === false &&
	...
	static actionLinkSubmit(instanceId, selectedElement) {
		selectedElement.parentElement.submit();
	}
	...
	static actionVideo(instanceId) {
		...
		const figureId = video.getAttribute('data-figure');

		EditorVideo.convert(instanceId, figureId, video.value);
	...
	static actionEmbed(instanceId, selectedElement) {
		EditorEmbed.convert(instanceId, selectedElement);
	}
	...
	static action(action, instanceId, selectedElement) {
		...
		switch(action) {
			...
			case 'link-submit' :
				return this.actionLinkSubmit(instanceId, selectedElement);
			...
			case 'embed' :
				return this.actionEmbed(instanceId, selectedElement);
	...
	static configure(instanceId, mediaSelector) {
		...
		new Ajax.Query(mediaSelector)
			...
			.body({
				...
				license: mediaSelector.getAttribute('data-license'),
				source: mediaSelector.getAttribute('data-source'),
	...
	static _header(instanceId) {
		...
		const newHeaderSize = firstNodeP.hasAttribute('data-header') ? null : 1;
	...
	static _displayLink(instanceId, node) {
		...
		if(node !== null) {
			...
			qs(instanceId +'-box-selection [data-action="link-submit"]').innerHTML = Editor.labels.linkFilledSubmit;
			...
			qs(instanceId +'-box-selection [data-action="link-submit"]').innerHTML = Editor.labels.linkEmptySubmit;
	...
	static _restyleIcons(instanceId) {
		...
		if(qs(instanceId).getAttribute('data-text-full') === '1') {
...
class EditorFigure {
	...
	static select(instanceId, figureSelector) {
				...
				html += '<div class="editor-box-figure-container">';
					...
					html += '<div>';
						...
						html += '<div class="dropdown-list">';
							html += '<button class="editor-action" data-action="resize" data-size="compressed" data-instance="'+ instanceId +'"><span>'+ Editor.labels.resizeCompress +'</span></button>';
							html += '<button class="editor-action" data-action="resize" data-size="left" data-instance="'+ instanceId +'"><span>'+ Editor.labels.resizeLeft +'</span></button>';
							html += '<button class="editor-action" data-action="resize" data-size="right" data-instance="'+ instanceId +'"><span>'+ Editor.labels.resizeRight +'</span></button>';
							html += '<button class="editor-action" data-action="resize" data-size="expanded" data-instance="'+ instanceId +'"><span>'+ Editor.labels.resizeExpand +'</span></button>';
						html += '</div>';
					html += '</div>';
		...
		qs(boxId +' [data-action="resize"][data-size="'+ figureSelector.getAttribute('data-size') +'"]').classList.add('selected');
	...
	static updateCaption(figureSelector) {
		...
		const span = figureSelector.qs('.editor-box-figcaption > span');
	...
	static create(instanceId, options) {
		...
		if(Editor.isEditorNode(nodeHost)) {
			...
			const nodeAncestor = Editor.getTopAncestor(nodeHost);

			if(EditorKeyboard.isEmptyLine(nodeAncestor)) {
				...
				nodeAncestor.insertAdjacentElement('afterend', node);
		...
		if(node.parentElement !== null) {
			...
			if(nodeFigure.nextSibling === null) {
				...
				nodeFigure.parentElement.insertAdjacentElement('afterend', nodeP);
	...
	static resize(instanceId, figureSelector, size) {
		...
		qsa(figureId +'-box-figure [data-action="resize"]', node => node.classList.remove('selected'));
		qs(figureId +'-box-figure [data-action="resize"][data-size="'+ size +'"]').classList.add('selected');
	...
	static reorganizeInstance(instanceId) {
		...
		qs(instanceId +' figure[data-interactive="true"]', figureSelector => {
			EditorFigure.reorganize(instanceId, figureSelector);
		});
...
// Handle embed in the editor
class EditorEmbed {

	static convert(instanceId, node) {

		if(qs(instanceId).hasAttribute('data-figure') === '0') {
			return;
		}

		EditorEmbed.link(instanceId, node);

	};

	// changes the external links into <a> tag with correct text
	static link(instanceId, node) {

		const url = node.nodeValue;

		if(url.match(/^(http[s]?:\/\/([=&#a-zA-Z0-9%\:\.\-\/\_\+\?]*))$/g) === null) {
			return false;
		}

		new Ajax.Query()
			.url('/editor/:getExternalLink')
			.body({
				url: url
			})
			.fetch()
			.then((json) => {
				...
				let html = '<div class="link-presentation">';

					html += '<div class="link-image" style="background-image: url('+ json.image +')"></div>';

					html += '<div class="link-content">';
						html += '<div class="link-display">';
							html += '<a href="' + json.link + '">' + json.title + '</a>';
							if(json.description !== '') {
								html += '<p>' + json.description + '</p>';
							}
						html += '</div>';
						html += '<div class="link-source">';
							html += json.url;
						html += '</div>';
					html += '</div>';
				...
				const nodeEmbed = document.createElement('div');
				nodeEmbed.setAttribute('class', 'editor-embed');
				nodeEmbed.innerHTML = html;
				...
				const attributes = {
					...
					'link-image-url': json.image
				};

				EditorFigure.addNode(instanceId, figureId, nodeEmbed, 'embed', null, null, attributes);

				const nodeAncestor = Editor.getTopAncestor(node);
				nodeAncestor.parentElement.removeChild(nodeAncestor);
...
class EditorQuote {
	...
	static create(instanceId) {
		...
		nodeImage.setAttribute('placeholder', Editor.labels['quoteQuote']);
		...
		EditorFigure.addNode(instanceId, figureId, nodeQuote, 'quote', null, null, {'about': 'quote'});
	...
	static next(imageSelector) {
		...
		let newAbout;
		if(quoteSelector.hasAttribute('data-about')) {
			newAbout = order[quoteSelector.getAttribute('data-about')];
		} else {
			newAbout = 'quote';
		}

		const newAboutLabel = 'quote' + newAbout.charAt(0).toUpperCase() + newAbout.slice(1);

		quoteSelector.setAttribute('data-about', newAbout);
		imageSelector.setAttribute('placeholder', Editor.labels[newAboutLabel]);
...
class EditorImage {
	...
	static upload(instanceId, figureId, e, files) {
		...
		function onLoad(resolve, file, img) {
		...
		switch(nodeTag) {
			...
			case 'PRE' :
				...
				if(
					...
					node.parentElement.tagName === 'BLOCKQUOTE'
				) {
					EditorNode.trim(node);
					return EditorMutation._nodeReplace(node, 'P');
				}
