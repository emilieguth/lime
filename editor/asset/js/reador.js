document.delegateEventListener('click', 'a[data-action="figure-extend"]', function(e) {
	...
	const media = this.firstParentMatches('div.editor-media');
...
class ReadorZoom {
	...
	static canZoom() {
		return (ReadorZoom.isZooming === false);
	}
	...
	static displayZoomedItem(item, positionCallback) {
		...
		const reador = item.firstParentMatches('.reador');
		...
		setTimeout(() => {
			...
			if(xyz) {
				...
				if(qs('div.editor-image-metadata[data-xyz="' + xyz + '"]') === null) {

					new Ajax.Query()
						.url('/paper/text:imageMetadata')
						.body({
							text: reador.id.substring(7),
							xyz: xyz
						})
						.fetch()
						.then((json) => {

							let metadataX = positionX;
							let metadataWidth = imageWidth;

							if((screenWidth - imageWidth) < 10) {

								const metadataCrop = (10 - (screenWidth - imageWidth));

								metadataX += metadataCrop;
								metadataWidth -= metadataCrop * 2;

							}

							item.insertAdjacentHTML('afterend', json.metadata);

							qs('div.editor-image-metadata[data-xyz="' + xyz + '"]', metadata => {

								metadata.style.marginLeft = metadataX +'px';
								metadata.style.width = metadataWidth +'px';
...
class ReadorZoomNavigator {
	...
	static init(item) {
		...
		const parentItem = item.firstParentMatches('.editor-media[data-type="image"]');
