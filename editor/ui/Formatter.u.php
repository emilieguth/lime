...
abstract class FormatterUi {
	...
	protected function filterXml(string $value, array $options): \DOMElement {
		...
		for($i = 0; $i < $main->childNodes->length; $i++) {
			...
			switch($node->nodeName) {
				...
				case 'p' :
					$this->convertParagraphNode($node);
					break;

				case 'header' :
					$this->convertHeaderNode($node);
					break;

				case 'ol' :
				case 'ul' :
					$this->convertListNode($node);
					break;

				case 'figure' :
					$i += $this->convertFigureNode($node);
					break;
			...
			if($childNode->nodeType !== XML_ELEMENT_NODE) {
				...
				switch($childNode->nodeName) {
					...
					case 'embed' :
						...
						$newChildNode->setAttribute('data-source', $childNode->getAttribute('source'));
						$newChildNode->setAttribute('data-url', $childNode->getAttribute('url'));

						switch($childNode->getAttribute('source')) {

							case 'link' :
								$this->convertEmbedLinkNode($childNode, $newChildNode);
								break;

							case 'twitter' :
								$this->convertEmbedTwitterNode($childNode, $newChildNode);
								break;
	...
	protected function convertImageNode(\DOMNode $node, \DOMNode $newNode, int $position, array $orientations, string $figureSize) {
		...
		if($this->for === 'editor') {
			...
			$newNode->setAttribute('data-license', $node->getAttribute('license'));
	...
	protected function getImageFormat(string $xyz, int $position, string $orientation, int $figureCount, string $figureSize): string {
		...
		if($isMobile) {
			...
			switch($figureSize) {
				...
				// This is expanded, so it is big!
				case 'expanded' :

					if($lineCount === 1 or $lineCount === 2) {
						return 'xl';
					} else {
						return 'l';
					}
	...
	public function getImageMetadata(\DOMNode $node) {

		$h = '<div data-xyz="'.$node->getAttribute('xyz').'" class="editor-image-metadata text-color">';
			$h .= $this->buildTitle($node);
			$h .= '<div class="editor-image-metadata-info">';
				$h .= $this->buildDownload($node);
				$h .= $this->buildLicense($node, TRUE);
			$h .= '</div>';
		$h .= '</div>';
	...
	protected function buildTitle(\DOMNode $node): string {

		$h = '<div class="editor-image-metadata-title">';
	...
	protected function buildLicense(\DOMNode $node, $asHtml): string {

		if($node->getAttribute('license') === '') {
			$license = LicenseUi::getInfo('copyright', 'text');
		} else {
			$license = $node->getAttribute('license');
		}

		if($asHtml) {
			return '<div class="editor-image-metadata-license">'.$license.'</div>';
		} else {
			return $license;
		}
	...
	protected function convertQuoteNode(\DOMNode $node, \DOMNode $newNode) {
		...
		if($this->for === 'editor') {
			...
			$divImageNode->setAttribute('placeholder', (new \editor\EditorUi())->labels()['quote'.ucfirst($about)]);
		...
		while($node->childNodes->length > 0) {
			...
			// Only <p> and <header> as children for blockquote
			switch($childNode->nodeName) {

				case 'p' :
					$this->convertParagraphNode($childNode);
					break;

				case 'header' :
					$this->convertHeaderNode($childNode);
					break;

			}

			$blockquoteNode->appendChild($childNode);
	...
	/**
	 * Create an embed node for a figure
	 *
	 * @param \DOMNode $node
	 * @param \DOMNode $newNode
	 */
	protected function convertEmbedLinkNode(\DOMNode $node, \DOMNode $newNode) {

		$description = $node->getAttribute('link-description');

		$xml = '<div class="link-presentation">';

			$xml .= '<div class="link-image" style="background-image: url('.encode($node->getAttribute('link-image-url')).')"></div>';

			$xml .= '<div class="link-content">';
				$xml .= '<div class="link-display">';
					$xml .= '<a href="'.encode($node->getAttribute('url')).'" rel="nofollow">'.encode($node->getAttribute('link-title')).'</a>';
					if($description) {
						$xml .= '<p>'.encode($description).'</p>';
					}
				$xml .= '</div>';
				$xml .= '<div class="link-source">';
					$xml .= '<a href="'.encode($node->getAttribute('url')).'" rel="nofollow">'.encode($node->getAttribute('url')).'</a>';
				$xml .= '</div>';
			$xml .= '</div>';

		$xml .= '</div>';

		$embedNode = $this->dom->createDocumentFragment();
		$embedNode->appendXML($xml);

		$newNode->appendChild($embedNode);

		if($this->for === 'editor') {
			$newNode->setAttribute('data-link-title', $node->getAttribute('link-title'));
			$newNode->setAttribute('data-link-description', $node->getAttribute('link-description'));
			$newNode->setAttribute('data-link-image-url', $node->getAttribute('link-image-url'));
		}

	}

	/**
	 * Create an embed twitter for a figure
	 *
	 * @param \DOMNode $node
	 * @param \DOMNode $newNode
	 */
	protected function convertEmbedTwitterNode(\DOMNode $node, \DOMNode $newNode) {

		// Convert twitter card into valid XML
		$dom = new \DOMDocument();
		$dom->strictErrorChecking = FALSE;

		$dom->loadHtml(
			'<?xml version="1.0" encoding="UTF-8"?>'.$node->getAttribute('html'),
			LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOERROR | LIBXML_NOXMLDECL
		);
		...
		for($i = 1 /* Ignore XML declaration */; $i < $dom->childNodes->length; $i++) {
			$xml .= $dom->saveXml($dom->childNodes[$i]);
		...
		if($this->for === 'editor') {
			$newNode->setAttribute('data-html', $node->getAttribute('html'));
		}
	 ...
	 * @param \DOMNode $node
	 */
	protected function convertParagraphNode(\DOMNode $node) {
	 ...
	 * @param \DOMNode $node
	 */
	protected function convertHeaderNode(\DOMNode $node) {
	...
	protected function convertLinkNode(\DOMNode $node) {
		...
		if($this->options['links'] ?? TRUE) {
			...
			// External links have a target=_blank
			if(
				preg_match('/^https?/si', $url) > 0 and
				strpos($url, \Lime::getUrl()) !== 0
			) {
	...
	private function convertListNode(\DOMNode $node) {
