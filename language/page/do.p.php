...
(new Page(function($data) {
	...
	if($langs === '') {
		throw new FailAction("Missing selected languages langs=?");
	} else if($langs === '*') {
		...
		array_map(function($lang) {
			...
			if(!preg_match('/^[a-z]{2}\\_[A-Z]{2}$/', $lang)) {
				throw new FailAction("Language '".$lang."' is invalid.");
			}
	...
	if(!preg_match('/^[a-z]{2}\\_[A-Z]{2}$/', $langSource)) {
		throw new FailAction("Source language '".$langSource."' is invalid.");
	}
