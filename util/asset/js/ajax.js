...
Ajax.Query = class {
	...
	lock() {
		...
		if(this.elementsLocked[lockId] !== undefined) {
			throw 'LOCK_FAILED';
		}

		// Lock
		this.elementsLocked[lockId] = setTimeout(() => this.unlock(), 1000);
	...
	unlock() {
		...
		if(typeof this.elementsLocked[lockId] === 'undefined') {
			return false;
		}

		clearTimeout(this.elementsLocked[lockId]);
		this.elementsLocked[lockId] = undefined;
	...
	renderOk(json) {
		...
		return new Promise(resolve => {
			...
			const errors = new Ajax.Instruction(json, "ajax", this.context).render();
...
Ajax.Navigation = class extends Ajax.Query {
	...
	renderOk(json) {
		...
		return new Promise(resolve => {
			...
			if(json.__context === 'new') {
				this.isCleanLayers = true;
			}

			const promise = this.isCleanLayers ?
	...
	showWaiter(zIndex) {
		...
		setTimeout(() => {
			qs('#'+ this.isWaiter).style.opacity = 1;
		}, 10);
	...
	renderRedirect(__instructions, __redirect) {
		...
		switch(mode) {
			...
			case 'ajaxNavigation' : {
				...
				const query = new Ajax.Navigation(this.context);

				query
					.purgeLayers(cleanLayers)
					.url(url)
					.fetch()
			...
			switch(name) {
				...
				case 'remove' :
					...
					if(options.delay !== undefined) {
						setTimeout(() => node.remove(), options.delay);
					} else {
						node.remove();
					}
 ...
 * Ajax handling
 */
document.delegateEventListener('click', 'a[data-ajax], button[data-ajax]', function(e) {
...
document.delegateEventListener('click', 'a[href]', function(e) {
	...
	if(this.getAttribute('data-ajax-navigation') === 'no') {
		return true;
	}
