...
class SessionLib {
	...
	protected static function createSession(string $sid): bool {
		...
		if($affected > 0) {
			...
			self::getCache()->set('session-'.$sid, self::$content, self::REGENERATION * 2);
			return TRUE;
	...
	protected static function deleteSession(string $sid): void {
		...
		self::getCache()->delete('session-'.$sid);
	...
	public static function read(string $sid): string {
		...
		$result = self::getCache()->get('session-'.$sid);
	...
	public static function write(string $sid, string $content): bool {
		...
		if($affected === 0) {
			...
			self::getCache()->set('session-'.$sid, $content, self::REGENERATION);
		}
